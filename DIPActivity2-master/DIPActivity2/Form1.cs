...
namespace DIPActivity2
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            frameTimer.Tick += new EventHandler(UpdateFrame);
        }
        private void UpdateFrame(object sender, EventArgs e)
        {
            if (firstDevice != null)
            {
                firstDevice.CaptureFrameToClipboard();
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            // Also ensure the webcam is properly closed
            if (firstDevice != null)
            {
                firstDevice.Stop();
            }
        ...
        private void subtractionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            output = PerformImageSubtraction(image, background);
            pictureBox3.Image = output;
        }

        private Bitmap PerformImageSubtraction(Bitmap image, Bitmap background)
        {
            Bitmap output = new Bitmap(image.Width, image.Height);
            Color bgGreen = Color.FromArgb(0, 0, 255);
            int green = (bgGreen.R + bgGreen.G + bgGreen.B) / 3;
            ...
            for (int i = 0; i < image.Width; i++)
            {
                ...
                {
                    Color imageColor = image.GetPixel(i, j);
                    int grey = (imageColor.R + imageColor.G + imageColor.B) / 3;
                    int subtract = Math.Abs(grey - green);

                    output.SetPixel(i, j, subtract < greenThreshold ? background.GetPixel(i, j) : imageColor);
                }
            }
